...
#beta GE = 0
path<-"/users/zwang4/family/simulation_gxe/05232024/power/e1/"
...
for(i in 1:sim_N){
  ...
  res_fam_size1[[i]]=PGS.TRI(PRS_fam[,1], PRS_fam[,2], PRS_fam[,3],formula= ~ E_sim_bin,E = dat$E_sim[id2,], GxE_int = T)
  fit <- glm(D ~ G_logistic+E[,1]+G_logistic:E[,1], family = binomial(), model = FALSE, x = FALSE, y = FALSE)
  res_glm_fam_size1[[i]]=summary(fit)$coef
  
  res_caseonly_size1[[i]]=function_caseonly(prs_c = PRS_fam[,1],envir = dat$E_sim[id2,1],mean_prs=mean(dat$pgs_fam[,1]))
  ...
  res_fam_size2[[i]]=PGS.TRI(PRS_fam[,1], PRS_fam[,2], PRS_fam[,3],formula= ~ E_sim_bin,E = dat$E_sim[id2,], GxE_int = T)
  fit <- glm(D ~ G_logistic+E[,1]+G_logistic:E[,1], family = binomial(), model = FALSE, x = FALSE, y = FALSE)
  res_glm_fam_size2[[i]]=summary(fit)$coef
  res_caseonly_size2[[i]]=function_caseonly(prs_c = PRS_fam[,1],envir = dat$E_sim[id2,1],mean_prs=mean(dat$pgs_fam[,1]))
  ...
  res_fam_size3[[i]]=PGS.TRI(PRS_fam[,1], PRS_fam[,2], PRS_fam[,3],formula= ~ E_sim_bin ,E = dat$E_sim[id2,], GxE_int = T)
  fit <- glm(D ~ G_logistic+E[,1]+G_logistic:E[,1], family = binomial(), model = FALSE, x = FALSE, y = FALSE)
  res_glm_fam_size3[[i]]=summary(fit)$coef
  res_caseonly_size3[[i]]=function_caseonly(prs_c = PRS_fam[,1],envir = dat$E_sim[id2,1],mean_prs=mean(dat$pgs_fam[,1]))
  ...
  res_fam_size4[[i]]=PGS.TRI(PRS_fam[,1], PRS_fam[,2], PRS_fam[,3],formula= ~ E_sim_bin,E = dat$E_sim[id2,], GxE_int = T)
  fit <- glm(D ~ G_logistic+E[,1]+G_logistic:E[,1], family = binomial(), model = FALSE, x = FALSE, y = FALSE)
  res_glm_fam_size4[[i]]=summary(fit)$coef
  res_caseonly_size4[[i]]=function_caseonly(prs_c = PRS_fam[,1],envir = dat$E_sim[id2,1],mean_prs=mean(dat$pgs_fam[,1]))
