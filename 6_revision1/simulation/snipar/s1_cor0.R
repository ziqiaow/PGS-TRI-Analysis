...
for(i in 1:sim_N){
  set.seed(i)
  dat=sim_prospective_population(pgs_c = pgs_children_kid1$SCORE1_SUM,pgs_m = pgs_m$SCORE1_SUM,pgs_f = pgs_f$SCORE1_SUM,cor_strat=cor_strat1,alpha_fam=-4.8,betaG_normPRS=beta_prs,beta_M=beta_mother,beta_F=beta_father)
...
save(res_fam_size1,res_fam_parent_size1,res_glm_fam_size1,res_ptdt_size1,
     file=paste0(path,"s1_direct0_indirect0_cor0",".rda"))
