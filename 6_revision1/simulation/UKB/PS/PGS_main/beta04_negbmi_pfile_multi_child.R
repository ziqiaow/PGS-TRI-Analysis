...
#load("/dcs04/nilanjan/data/zwang/family/simulation/new_score/PGS002440_clump/pgs/trimmed_data_pfile.RData")
...
# e_strat = pheno_father$townsend_index
# e_strat[which(is.na(e_strat))] = pheno_mother$townsend_index[which(is.na(e_strat))]
# e_strat = scale(e_strat)




#identical(id_parental$X1,as.character(pheno_father$ID))
#[1] TRUE


# mu_sim = scale(pheno_father$birth.y)
# #tmp = scale(children_pc$rotation[,1:200])
# tmp = apply(children_pc$rotation[,1:200], 1, sum) 
# mu_sim = mu_sim +tmp
...
# load("C:/Users/ziqia/Desktop/work/family/simulation/UKB/child_geno2.RData")
...
for(i in 1:125){
  ...
  dat = sim_prospective_population(pgs_c = pgs_c , pgs_f = pgs_f,pgs_m = pgs_m,e_strat = bmi,cor_strat = 2,alpha_fam = -6.7,pc=F,mu_pc = mu_sim,betaG_normPRS=0.4)
...
save(res_fam_size1,res_glm_fam_size1,res_ptdt_size1,res_pc_father,res_pc_mother,res_glm_fam_mf,
     res_glm_pc10_fam_size1,res_glm_pc10_geo_fam_size1,res_pc_famo,file=paste0(path,"res_beta04_size1k_trim_multichild",".rda"))
...
for(i in 1:125){
  ...
  # load(paste0("/dcs04/nilanjan/data/zwang/family/simulation/ukb_05052024/child_geno/child_pc_",i,".RData"))
  
  
  #first without PCs in the intercept term
  dat = sim_prospective_population(pgs_c = pgs_c , pgs_f = pgs_f,pgs_m = pgs_m,e_strat = bmi,cor_strat = 2,alpha_fam = -6.1,pc=F,mu_pc = mu_sim,betaG_normPRS=0.4)
...
save(res_fam_size1,res_glm_fam_size1,res_ptdt_size1,res_pc_father,res_pc_mother,res_glm_fam_mf,
     res_glm_pc10_fam_size1,res_glm_pc10_geo_fam_size1,res_pc_famo,file=paste0(path,"res_beta04_size2k_trim_multichild",".rda"))
