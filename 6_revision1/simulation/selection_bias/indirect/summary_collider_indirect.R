...
setwd("~/work/family/simulation/collider_bias/")
path<-"~/work/family/simulation/collider_bias/"
...
files <- c(list.files(pattern = "^s5.*\\.rda$"),list.files(pattern = "^s6.*\\.rda$"),list.files(pattern = "^s7.*\\.rda$"),list.files(pattern = "^s8.*\\.rda$"))

true_beta=c(0,0.4,0,0.4,0,0.4,0,0.4)
true_delta = 0
...
for(i in files){
    ...
    tmp[5,]=apply(res_estimate_parentindirect_size1,2,mean)-true_delta
    ...
    tmp[5,]=mean((res_estimate_parentindirect_size1-true_delta)^2)
    ...
    tmp[5,1]=mean(ifelse(true_delta<=upper[,1] & true_delta>=lower[,1],1,0))
