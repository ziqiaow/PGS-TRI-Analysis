...
true_beta=c(0,0,0.4,0.4,0,0,0.4,0.4,0,0,0,0,0.4,0.4)
...
for(i in files){
    ...
    tmp=data.frame(tmp)
    tmp$method=c("fam","logistic","ptdt")
    tmp$samplesize=rep(1000,3)
...
save(summary_mse,summary_bias,summary_empirical_sd,summary_sd,summary_type1error_power,summary_ci,file=paste0(path,"results_summary.rda"))
