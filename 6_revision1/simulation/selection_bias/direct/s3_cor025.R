#scenario 3: no population stratification bias
#PGS -> E1, E2 -> D, E1-> Z (selection), E2-> Z (selection), E1, E2 independent
...
for(i in 1:sim_N){
  set.seed(i)
  dat=sim_prospective_population_select(n_fam=600000,cor_strat=cor_strat1,alpha_fam=-5.5,betaG_normPRS=beta_prs, beta_select_E1=0.4,beta_select_E2=0.4, cor_e_prs=T,betaE1=0, betaE2=0.4)
  ...
  E1 = c(as.numeric(as.vector(dat$E_sim[id1,1])),as.numeric(as.vector(dat$E_sim[id2,1])))
  E2 = c(as.numeric(as.vector(dat$E_sim[id1,2])),as.numeric(as.vector(dat$E_sim[id2,2])))
  ...
  fit <- glm(D ~ G_logistic + E1 + E2, family = binomial(), model = FALSE, x = FALSE, y = FALSE)
...
save(res_fam_size1,res_glm_fam_size1,res_ptdt_size1,
     file=paste0(path,"s3_pgs0_cor025",".rda"))
