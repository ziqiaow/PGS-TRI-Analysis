#scenario 4: no population stratification bias
#PGS -> D, E -> D, E-> Z (selection), PGS and E cor = 0.2, PGS cause E
...
for(i in 1:sim_N){
  set.seed(i)
  dat=sim_prospective_population_select(n_fam=600000,cor_strat=cor_strat1,alpha_fam=-5.5,betaG_normPRS=beta_prs, beta_select_E1=0.4,beta_select_E2=0, cor_e_prs=T,betaE1=0.4, betaE2=0, PGScauseE = T)
  ...
  fit <- glm(D ~ G_logistic + E, family = binomial(), model = FALSE, x = FALSE, y = FALSE)
...
save(res_fam_size1,res_glm_fam_size1,res_ptdt_size1,
     file=paste0(path,"s4_pgs04_cor025",".rda"))
