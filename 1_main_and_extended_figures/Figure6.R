...
p1<-ggplot(
  ...
  scale_y_continuous(limits = c(0.15,4),breaks = c(0.1,1,2,3,4))+
...
p <- ggarrange(p1,
               ggarrange(p3,p4,ncol = 2, labels = c("b",NA), font.label = list(size = 7),widths = c(0.9,0.1)),
               nrow = 2, 
               labels = c("a", NA), font.label = list(size = 7),
               heights = c(0.65,0.35)
)

ggsave(filename=paste0("Figure6.pdf"),
       plot=p, 
       path="./family/manuscript/draft4/plots/main/",
       width=180, height=180, units="mm", dpi=1000,device = cairo_pdf)

ggsave(filename=paste0("Figure6.png"),
       plot=p, 
       path="./family/manuscript/draft4/plots/main/",
