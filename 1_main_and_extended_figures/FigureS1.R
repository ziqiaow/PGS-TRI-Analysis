...
p1 <- ggplot(data=data_plot, aes(x=Family_Size, y=beta_prs , fill=Method)) +
  geom_bar(stat="identity", position=position_dodge(),color="black",width = 0.8,linewidth = 0.1)+
  ...
  geom_hline(yintercept=95, col = "coral",linetype = "dashed",size=0.5)  + 
  facet_grid(cor~truebeta)+
  labs(title="Direct PGS Effect") +
   theme_bw()+      
...
p2 <- ggplot(data=data_plot, aes(x=Family_Size, y=coverage_prob , fill=Method)) +
  ...
  scale_y_continuous('95% Coverage Probability (%)',breaks = c(0,50,95),limits = c(0,100))+
  geom_hline(yintercept=95, col = "coral",linetype = "dashed",size=0.5)  + 
  facet_grid(cor~truedelta)+ 
 labs(title="Parental Differential Indirect PGS Effect") +
   theme_bw()+      
  theme(plot.title = element_text(color = "black", size = 7, face = "bold")) +                                                          
  My_Theme+theme(legend.position="") 
...
#load("./family/simulation_gxe/05232024/prs_e_cor0_05282024/results_summary.rda")
...
p3 <- ggplot(data=data_plot, aes(x=Family_Size, y=beta_prs_e1 , fill=Method)) +
  ...
  labs(title="PGS x E1 Interaction") + theme_bw()+      
  theme(plot.title = element_text(color = "black", size = 7, face = "bold")) +                                                          
  My_Theme+theme(legend.position="") 
...
p4 <- ggplot(data=data_plot, aes(x=Family_Size, y=beta_prs_e2 , fill=Method)) +
  ...
  labs(title="PGS x E2 Interaction") + theme_bw()+      
  theme(plot.title = element_text(color = "black", size = 7, face = "bold")) +                                                          
  My_Theme+theme(legend.position="") 
...
p_tmp <-  ggplot(data=data_plot, aes(x=Family_Size, y=beta_prs_e2 , fill=Method)) +
  ...
  geom_hline(yintercept=95, col = "coral",linetype = "dashed",size=1)  + 
  facet_grid(strat~cor)+theme_bw()+      
          theme(plot.title = element_text(color = "black", size = 7, face = "bold", hjust = 0.5)) +                                                          
          My_Theme

p_tmp <- p_tmp+  theme(legend.text = element_text(face = "bold", size = 7),legend.title = element_text(face = "bold", size = 7),legend.key.size = unit(3, 'mm'))
...
ggsave(filename=paste0("Extended_Figure1.pdf"),
       plot=p, 
       path="./family/manuscript/draft4/plots/extended/",
       width=180, height=175, units="mm", dpi=400,device = cairo_pdf)

ggsave(filename=paste0("Extended_Figure1.png"),
       plot=p, 
       path="./family/manuscript/draft4/plots/extended/",
       width=180, height=175, units="mm", dpi=400)
