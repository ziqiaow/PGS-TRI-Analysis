...
#pcd_sample <- pcd_sp %>%
#  slice_sample(prop = 0.5)  # Keep 50% of points

#Remove duplicate coordinates to reduce the plot size
pcd_sample <- pcd_sp %>%
...
p1 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = PGS),size = 0.1)+
  scale_color_viridis_c(option = "A")+ My_Theme

p2 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = BMI),size = 0.1)+
  scale_color_viridis_c(option = "A")+  My_Theme


p3 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = Education),size = 0.1)+
  scale_color_viridis_c(option = "A")+  My_Theme


p4 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = Townsend),size = 0.1)+
  scale_color_viridis_c(option = "A")+  My_Theme

p_pc1 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = PC1),size = 0.1)+
  scale_color_viridis_c(option = "D")+  My_Theme

p_pc2 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = PC2),size = 0.1)+
  scale_color_viridis_c(option = "D")+  My_Theme

p_pc3 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = PC3),size = 0.1)+
  scale_color_viridis_c(option = "D", breaks = seq(-0.5, 0.4, by = 0.2),  # Sequence of breaks
                        limits = c(-0.5, 0.4))+  My_Theme

p_pc4 <- ggplot(pcd_sample) + 
  geom_sf(aes(color = PC4),size = 0.1)+
  scale_color_viridis_c(option = "D", breaks = seq(-1, 2, by = 1),  # Sequence of breaks
                        limits = c(-1, 2))+ My_Theme
...
pg2$widths <- pg1$widths
pg3$widths <- pg1$widths
pg4$widths <- pg1$widths
pg_pc1$widths <- pg1$widths
pg_pc2$widths <- pg1$widths
pg_pc3$widths <- pg1$widths
pg_pc4$widths <- pg1$widths

p <- ggarrange(ggarrange(pg1,pg2,pg3,pg4,
                         ncol = 4, labels = c("a", "b", "c","d"),font.label = list(size = 7),
                         widths = c(0.25,0.25,0.25,0.25),heights = c(0.25,0.25,0.25,0.25),align = "hv") , 
               ggarrange(pg_pc1,pg_pc2,pg_pc3,pg_pc4,
                         ncol = 4, labels = c("e", "f", "g","h"),font.label = list(size = 7),
                         widths = c(0.25,0.25,0.25,0.25),heights = c(0.25,0.25,0.25,0.25),align = "hv") ,
               nrow = 2, 
               labels = c(NA, NA),font.label = list(size = 7),
               heights = c(0.5,0.5),align = "hv")

ggsave(filename=paste0("Extended_Figure4.pdf"),
       plot=p, 
       path="./family/manuscript/draft4/plots/extended/",
       width=180, height=120, units="mm", dpi=320,device = cairo_pdf)
